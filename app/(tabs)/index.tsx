...
import {
  ...
  useNotifications, useInsertNotification,
} from '@/lib/hooks';
...
function OwnerDashboard() {
  ...
  // ── Membership expiry notifications (1 day before) ─────────────────
  // Track which member IDs we've already notified this session to avoid duplicates
  const notifiedRef = useRef<Set<string>>(new Set());

  useEffect(() => {
    if (!user?.gym_id || members.length === 0) return;

    const today = new Date();
    today.setHours(0, 0, 0, 0);

    members.forEach((m: any) => {
      if (!m.expiry_date || !m.id) return;

      const expiry = new Date(m.expiry_date);
      expiry.setHours(0, 0, 0, 0);
      const diffDays = Math.round((expiry.getTime() - today.getTime()) / 86400000);

      // Notify exactly 1 day before expiry
      if (diffDays === 1 && !notifiedRef.current.has(m.id)) {
        notifiedRef.current.add(m.id);
        insertNotification.mutate({
          gym_id: user.gym_id,
          member_id: m.id,
          title: 'Membership Expiring Tomorrow',
          body: `Hi ${m.name}, your membership expires tomorrow (${new Date(m.expiry_date).toLocaleDateString()}). Please renew to continue enjoying gym services.`,
    ...
    <>
      ...
      >
        <View style={styles.statsSection}>
          ...
          <View style={styles.statsGrid}>
            <Pressable style={{ flex: 1 }} onPress={() => router.push({ pathname: '/(tabs)/members', params: { filter: 'active' } } as any)}>
              <StatCard label="Active Members" value={activeMembers} color={Colors.primary} />
            </Pressable>
            <Pressable style={{ flex: 1 }} onPress={() => router.push({ pathname: '/(tabs)/members', params: { filter: 'expiring' } } as any)}>
...
function MemberDashboard() {
  ...
  const { data: notifs = [] } = useNotifications(user?.member_id, user?.gym_id, user?.role === 'trainer' ? user?.id : null);
