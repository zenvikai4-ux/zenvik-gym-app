...
export default function TrainersScreen() {
  ...
  // Guard: only gym_owner should access this screen
  const gymId = user?.role === 'super_admin' ? null : user?.gym_id;

  const { data: trainers = [], isLoading } = useTrainers(gymId);
  ...
  const handleAdd = async () => {
    ...
    const hasLoginCredentials = form.email && form.password && form.password.length >= 6;
    const systemEmail = form.email || `trainer_${Date.now()}@${gymId?.slice(0,8)}.gymapp.local`;
    const systemPassword = hasLoginCredentials ? form.password : `sys_${Date.now()}_${Math.random().toString(36).slice(2)}`;
    ...
    insertTrainer.mutate({
      ...
      onSuccess: () => {
        ...
        insertActivity.mutate({
          ...
          details: hasLoginCredentials ? `${form.name} (login created)` : form.name,
        });
        setShowAdd(false);
  ...
  const handleEdit = async () => {
    ...
    setSaving(true);
    const updateData: any = { name: editForm.name, phone: editForm.phone || null, specialization: editForm.specialization || null };
    if (editForm.email) updateData.email = editForm.email;
    await supabase.from('profiles').update(updateData).eq('id', editingTrainer.profile_id || editingTrainer.id);
    if (newPassword && editForm.email) {
      await supabase.rpc('set_user_password' as any, { user_email: editForm.email, new_password: newPassword }).maybeSingle();
    }
    qc.invalidateQueries({ queryKey: ['trainers'] });
    setSaving(false);
    setNewPassword('');
    ...
    <View style={styles.container}>
                ...
                <View style={styles.cardActions}>
                  <Pressable
                    ...
                    onPress={() => {
                      ...
                      setEditForm({ name: item.name, email: item.email || '', phone: item.phone || '', specialization: item.specialization || '' });
                      setNewPassword('');
                      setChangePassError('');
                    }}
                  >
      ...
      <Modal visible={showAdd} animationType="slide" presentationStyle="formSheet" onRequestClose={() => setShowAdd(false)}>
        <View style={[styles.modal, { paddingTop: insets.top + 16 }]}>
          ...
          <ScrollView showsVerticalScrollIndicator={false}>
            ...
            {/* Login credentials - always shown, required when Trainer Login module is enabled */}
            <View style={[styles.loginSection, { borderColor: trainerLoginEnabled ? Colors.primary + '30' : Colors.border, backgroundColor: trainerLoginEnabled ? Colors.primary + '10' : Colors.secondary }]}>
              <View style={styles.loginHeader}>
                <Ionicons name="phone-portrait-outline" size={16} color={trainerLoginEnabled ? Colors.primary : Colors.textMuted} />
                <Text style={[styles.loginTitle, { color: trainerLoginEnabled ? Colors.primary : Colors.text }]}>App Login Credentials</Text>
                <View style={[styles.moduleTag, { backgroundColor: trainerLoginEnabled ? Colors.primary + '20' : Colors.secondary }]}>
                  <Text style={[styles.moduleTagText, { color: trainerLoginEnabled ? Colors.primary : Colors.textMuted }]}>{trainerLoginEnabled ? 'Required' : 'Optional'}</Text>
                </View>
              </View>
              <Text style={styles.loginSub}>{trainerLoginEnabled ? 'Trainer will use these to log in to the app' : 'Optionally assign login credentials for this trainer'}</Text>
              <View style={styles.formField}>
                <Text style={styles.formLabel}>Email {trainerLoginEnabled ? '*' : '(optional)'}</Text>
                <TextInput
                  style={styles.formInput}
              ...
              <View style={styles.formField}>
                <Text style={styles.formLabel}>Password {trainerLoginEnabled ? '*' : '(optional)'}</Text>
      ...
      <Modal visible={!!editingTrainer} animationType="slide" presentationStyle="formSheet" onRequestClose={() => setEditingTrainer(null)}>
        <View style={[styles.modal, { paddingTop: insets.top + 16 }]}>
          ...
          {[
            { key: 'name', label: 'Name', keyboard: 'default', capitalize: 'words' },
            { key: 'phone', label: 'Phone', keyboard: 'phone-pad', capitalize: 'none' },
            { key: 'specialization', label: 'Specialization', keyboard: 'default', capitalize: 'words' },
            { key: 'email', label: 'Email (login)', keyboard: 'email-address', capitalize: 'none' },
          ].map(f => (
            <View key={f.key} style={styles.formField}>
              <Text style={styles.formLabel}>{f.label}</Text>
              <TextInput
          ...
          {/* Change Password */}
          <View style={[styles.loginSection, { marginTop: 4, borderColor: Colors.warning + '40', backgroundColor: Colors.warning + '08' }]}>
            <View style={styles.loginHeader}>
              <Ionicons name="key-outline" size={16} color={Colors.warning} />
              <Text style={[styles.loginTitle, { color: Colors.warning }]}>Change Password</Text>
              <View style={[styles.moduleTag, { backgroundColor: Colors.warning + '20' }]}>
                <Text style={[styles.moduleTagText, { color: Colors.warning }]}>Optional</Text>
              </View>
            </View>
            <Text style={styles.loginSub}>Leave blank to keep current password</Text>
            <View style={styles.formField}>
              ...
              <View style={styles.passRow}>
                ...
                />
                <Pressable onPress={() => setShowNewPass(s => !s)} style={{ padding: 10 }}>
